...
object ProxyTuning {
    ...
    // 曾有一个 UPSTREAM_SILENCE_MS = 90s 的「上游静默判死」阈值，已连同判据整个删除。
    // 两轮实测证明它抓到的全是连接池里的闲置连接（判死时客户端静默时长 p50 = 90s 整），
    // 且 264 条被判死的连接后来又收到了上游字节。详见 NioRelayReactor.sweepIdle 的说明。
    // 隧道回收现在只由用户可调的 idle 超时负责。
    /** accept backlog。 */
