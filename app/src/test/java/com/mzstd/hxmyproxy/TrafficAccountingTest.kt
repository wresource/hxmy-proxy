...
class TrafficAccountingTest {
    ...
    @Test fun accumulatesPerClientAndDomain() {
        ...
        acc.openConnection(ip1).also { it.bindHost("example.com"); it.add(100, 0); it.add(0, 200) }
    ...
    @Test fun globalSinkReceivesEveryIncrement() {
        ...
        acc.openConnection(ip1).also { it.bindHost("a.com"); it.add(50, 0); it.add(0, 70) }
        assertEquals(50, gUp); assertEquals(70, gDown)
    }

    @Test fun concurrentAddIsLossless() {
        val acc = TrafficAccounting()
        val t = acc.openConnection(ip1).also { it.bindHost("x.com") }
    ...
    @Test fun topNCapsDomainsWithOthersBucketAndConservesBytes() {
        ...
        listOf("a.com" to 10L, "b.com" to 20L, "c.com" to 30L).forEach { (h, b) ->
            acc.openConnection(ip1).also { it.bindHost(h); it.add(b, 0) }
        }
        val snap = acc.snapshot(10)
    ...
    @Test fun ageOutRemovesIdleClosedEntries() {
        ...
        acc.openConnection(ip1).also { it.bindHost("a.com"); it.add(10, 0); it.close() }
        now = 1000L + 10_000
        acc.ageOut(5_000)
    ...
    @Test fun resetClears() {
        val acc = TrafficAccounting()
        acc.openConnection(ip1).also { it.bindHost("a.com"); it.add(10, 10) }
        acc.reset()
        val snap = acc.snapshot(10)
